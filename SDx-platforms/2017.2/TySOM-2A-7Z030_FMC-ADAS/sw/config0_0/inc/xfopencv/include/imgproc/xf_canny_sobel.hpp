...
template<int ROWS,int COLS,int DEPTH_SRC,int DEPTH_DST,int NPC,int WORDWIDTH_SRC,int WORDWIDTH_DST, int WIN_SZ>
void xFSobel(
		...
		int _border_type,uint16_t imgheight,uint16_t imgwidth)
{
	...
	xFSobel3x3<ROWS,COLS,DEPTH_SRC,DEPTH_DST,NPC,WORDWIDTH_SRC,WORDWIDTH_DST,(COLS>> XF_BITSHIFT(NPC))+(WIN_SZ>>1),WIN_SZ, WIN_SZ*WIN_SZ>(_src, _dst,_dst1,WIN_SZ,imgheight,imgwidth);
